...
- Tables: `authors`, `book_categories`, `books`, `book_authors`, `book_category_links`, `book_files`, `book_course_links`, `favorite_books` (UNIQUE(user_id, book_id)); btree indexes cover ISBN, access type, language/year/cursor and link-table joins. Trigram/full-text index is deferred until the selected Serverpod version can own it without schema-check drift.
...
- [x] Policy matrix test PASS: `metadataOnly` không bao giờ trả nút Read/file URL cho role nào; `restricted` chỉ librarian/admin nhìn thấy đúng mức cấu hình.
- [x] Gõ nhanh 10 ký tự ⇒ chỉ 1 request sau debounce, kết quả stale bị hủy (test PASS).
- [x] Cuộn 100+ sách seed giả lập ⇒ cursor pagination, không tải toàn bộ (API test PASS).
- [x] Ảnh cover lỗi ⇒ placeholder + error state, không crash.
- [x] `flutter analyze`/`dart analyze`/tests PASS.
