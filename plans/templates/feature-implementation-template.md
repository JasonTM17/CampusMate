...
- **Replan authority**: User or evidence-backed Advisor/Kongming/Wukong/specialist.
- **Verification budget**: Focused checks while building; broad gates only at named checkpoints.
- **Spawn budget**: Keep required workflow roles; add no duplicate or speculative agents.
- **Deferred queue**: Record non-blocking findings without widening the release path.
