...
void main() {
  group('MessageBubble Citations', () {
    testWidgets('renders citation chips for assistant messages with citations', (
      tester,
    ) async {
      const message = ChatMessage(
        role: MessageRole.assistant,
        content:
            'Theo giáo trình giải thuật, tìm kiếm nhị phân có độ phức tạp O(log n).',
        citations: [
          ChatCitation(
      ...
      expect(
        find.text('Giáo trình Cấu trúc Dữ liệu - Chương 2, tr. 45'),
        findsOneWidget,
      );
      expect(
        find.text('Sổ tay Sinh viên 2026 - Quy chế, tr. 12'),
        findsOneWidget,
      );
    });

    testWidgets(
      'does not render citation section when citations list is empty',
      (tester) async {
        const message = ChatMessage(
          role: MessageRole.assistant,
    ...
    testWidgets(
      'clicking document citation without bookId opens detail dialog',
      (tester) async {
        const message = ChatMessage(
          role: MessageRole.assistant,
          ...
          citations: [
            ChatCitation(
              ...
              documentId: 5,
              quote:
                  'Sinh viên phải tích lũy tối thiểu 130 tín chỉ để tốt nghiệp.',
            ),
          ],
        );
        ...
        final chipFinder = find.text(
          'Sổ tay Sinh viên 2026 - Quy chế đào tạo, tr. 25',
        );
