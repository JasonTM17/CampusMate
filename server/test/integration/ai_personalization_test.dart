    ...
    (sessionBuilder, endpoints) {
      ...
      group('AI Memory and Blocklist', () {
        test('adds valid memory and retrieves it', () async {
          ...
          expect(
            memory.content,
            'Thích học bằng sơ đồ tư duy và ví dụ thực tế',
          );
          ...
          final listB = await endpoints.ai.getMemories(
            userB,
            activeOnly: false,
          );
        ...
        test('blocks memories containing passwords or secrets', () async {
          ...
          await expectLater(
            endpoints.ai.addMemory(
              userA,
              content:
                  'API token bí mật: ghp_123456789012345678901234567890123456',
            ),
            throwsA(
              isA<ServerpodClientException>().having(
                (e) => e.statusCode,
        ...
        test(
          'toggles and deletes memory with ownership verification',
          () async {
            final memory = await endpoints.ai.addMemory(
              userA,
      ...
      group('Student Context Isolation and Prompt Verification', () {
        test(
          'injects student context and enforces strict cross-user isolation',
          () async {
            ...
            expect(
              promptA,
              contains('ngắn gọn, đi thẳng vào trọng tâm'),
            ); // concise style
            ...
            expect(
              promptB,
              contains('Giải thích chi tiết, từng bước'),
            ); // detailed style
        ...
        test(
          'omits student context when personalization is turned off',
          () async {
            await endpoints.ai.updatePreferences(
              userA,
              explanationStyle: 'standard',
            ...
            expect(
              systemMsg,
              isNot(contains('[Personalized Student Context]')),
            );
          },
        );

        test(
          'includes book excerpt in chat payload without dumping entire book',
          () async {
            await endpoints.ai.updatePreferences(
              userA,
              explanationStyle: 'standard',
      ...
      group('Study Suggestion', () {
        test(
          'returns personalized study suggestion or null if personalization is off',
          () async {
            await endpoints.ai.updatePreferences(
              userA,
              explanationStyle: 'standard',
            ...
            final disabledSuggestion = await endpoints.ai.getStudySuggestion(
              userA,
            );
