...
void main() {
  group('UploadValidationService (Phase-11)', () {
    ...
    setUp(() {
      service = UploadValidationService(maxSizeBytes: 5 * 1024 * 1024); // 5MB for testing
    });
    ...
    test('neutralizes path traversal attempts in filename during ticket creation', () {
      final ticket = service.createUploadTicket(
        bookId: 99,
    ...
    test('validates valid EPUB bytes (PK zip header with epub structure)', () {
      ...
      final validEpubBytes = [
        ...
        ...utf8.encode('application/epub+zipMETA-INF/container.xml contents...'),
      ];
    ...
    test('NEGATIVE GATE: rejects evil.pdf containing non-PDF bytes (MIME mismatch)', () {
      final fakePdfBytes = utf8.encode('This is plain text pretending to be a PDF file.');
    ...
    test('NEGATIVE GATE: rejects file exceeding max size limit', () {
      final largeBytes = List<int>.filled(6 * 1024 * 1024, 0x25); // 6MB > 5MB limit
